...
> `main` 当前是已发布的 V0.4.0；`dev` 已进入 `0.5.0-dev.1`。V0.5 将在不破坏本地模式的前提下加入账号、情侣空间、云端同步、对象存储和离线队列。完整设计见 [V0.5.0 双人云端版架构](docs/V0.5.0_ARCHITECTURE.md)。

Vercel 仍是正式前端入口之一；Supabase / CloudBase 属于后端能力，不是 Vercel 的替代品。中国大陆场景当前优先采用 CloudBase PG，并保留 CloudBase 静态托管镜像。
