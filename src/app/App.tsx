...
import type { AppView, CompletionDetails, CustomWishInput, DateIdea, Wish } from '../domain/wish'
...
export function App() {
  ...
  const saveCompletion = (details: CompletionDetails) => {
    ...
    appState.completeWish(completionWish.id, details)
    setCompletionWish(null)
    if (!wasCompleted) celebrate('complete')
    notify(wasCompleted ? '这段回忆已经更新' : '这一天已经被好好保存')
  }
  const openSecret = () => {
  ...
  return (
    <AppShell view={view} completedCount={appState.stats.completed}
      ...
      {view === 'today' && <HomeScreen profile={appState.state.profile}
        dailyAnswer={appState.state.dailyAnswers[getDateKey()]}
        ...
        onOpenAddWish={() => setAddWishOpen(true)} onNavigate={navigate} onNotify={notify} />}
      {view === 'explore' && <ExploreScreen wishes={allWishes} categories={categories} categoryMap={categoryMap}
        ...
        onCelebrateSave={() => celebrate('save')} onNotify={notify} />}
      {view === 'collection' && <CollectionScreen wishes={allWishes} categories={categories} categoryMap={categoryMap}
        progress={appState.state.progress} savedCount={appState.stats.saved} completedCount={appState.stats.completed}
      ...
      {view === 'together' && <TogetherScreen profile={appState.state.profile} capsules={appState.state.capsules}
        completedCount={appState.stats.completed} photoCount={appState.stats.memoriesWithPhotos}
        ...
        onNotify={notify} />}
      {view === 'secret' && <SecretScreen openedAt={appState.state.secretOpenedAt} onBack={() => navigate('collection')} />}
      ...
      {completionWish && <CompletionModal wish={completionWish} progress={appState.state.progress[completionWish.id]}
